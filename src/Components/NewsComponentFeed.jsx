...
function NewsComponentFeed({ items, topic }) {
  return (
    <>
    <h2 className = "text-white text-3xl mt-2 mb-4 text-left ml-14">{topic}</h2>
    <div className="flex flex-wrap justify-center gap-4">
      {items.map((item, index) => (
        <a href={item.link} key={index} target="_blank" rel="noopener noreferrer" className="flex bg-stone-500 p-2 rounded-lg shadow-md mb-5 max-w-screen-sm">
          <div className="w-2/3">
